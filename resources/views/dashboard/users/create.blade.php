<x-dashboard title="{{ Breadcrumbs::current()->title }}">
    ...
    <form action="{{ route('users.store') }}" enctype="multipart/form-data" method="POST" class="d-flex">
        ...
        <div class="form-group col-2 mr-3">
            <img id="imageHolder" class="img-thumbnail" src="{{ asset('storage/'. config('app.default-image.user')) }}" width="{{ config('app.image-size.user.width') }}" height="{{ config('app.image-size.user.height') }}">
        ...
        <div>
            ...
            <div class="card flex-fill shadow p-3 mb-5 bg-white rounded">
                <h5 class="card-header">
                    <i class="fas fa-edit fa-user-plus mr-1"></i>
                    {{ trans('users.new') }}
                </h5>
                ...
                <div class="card-body">
                    <div class="form-row">
                        <div class="form-group col-6">
                            ...
                            @error('username')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @else
                                <small class="form-text text-muted">{{ trans('users.messages.input-username-help') }}</small>
                            @enderror
                        </div>

                        <div class="form-group col-6">
                            <label for="input-email">{{ trans('users.email') }}</label>
                            <input type="email" class="form-control @error('email') is-invalid @enderror" id="input-email" name="email" value="{{ old('email') }}" autofocus required>
                            @error('email')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-row">
                        <div class="form-group col-6">
                            ...
                            <input type="text" class="form-control @error('first_name') is-invalid @enderror" id="input-fname" name="first_name" value="{{ old('first_name') }}">
                            @error('first_name')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-6">
                            ...
                            <input type="text" class="form-control @error('last_name') is-invalid @enderror" id="input-lname" name="last_name" value="{{ old('last_name') }}">
                            @error('last_name')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-row">
                        <div class="form-group col-6">
                            ...
                            <select id="input-gender" class="form-control @error('gender') is-invalid @enderror" name="gender">
                                <option value="0" {{ old('gender') == 0 ? 'selected' : '' }}>{{ trans('users.male') }}</option>
                                <option value="1" {{ old('gender') == 1 ? 'selected' : '' }}>{{ trans('users.female') }}</option>
                            </select>
                            @error('gender')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="form-row">
                        <div class="form-group col-6">
                            ...
                            <input type="text" class="form-control @error('phone') is-invalid @enderror" id="input-phone" name="phone" value="{{ old('phone') }}">
                            @error('phone')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-6">
                            ...
                            <input type="text" class="form-control @error('address') is-invalid @enderror" id="input-address" name="address" value="{{ old('address') }}">
                            @error('address')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
